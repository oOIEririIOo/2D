...
public class RogueGoblinIdleState : IState
{
...
public class RougeGoblinPatrolState : IState
{
...
public class RogueGoblinChaseState : IState
{
    ...
    public override void LogicUpdate()
    {
        ...
        else if (currentEnemy.target != null && Mathf.Abs(Vector2.Distance(currentEnemy.transform.position, currentEnemy.target.transform.position)) > (Mathf.Abs(currentEnemy.transform.position.x - currentEnemy.rangedOffset.transform.position.x)) && currentEnemy.canRangedAtk)
        {
            currentEnemy.SwitchState(StateType.RangedAttack);
        }
...
public class RogueGoblinRangedAttackState : IState
{
    public override void OnEnter(FSM enemy)
    {
        enemy.anim.SetBool("run", false);
        currentEnemy = enemy;
        currentEnemy.anim.Play("rangedAttack");
    ...
    public override void OnExit()
    {
        currentEnemy.canRangedAtk = false;
    }
}

public class RogueGoblinAttackState : IState
{
    public override void OnEnter(FSM enemy)
    {
...
public class RogueGoblinHurtState : IState
{
    float hurtTime;
    public override void OnEnter(FSM enemy)
    {
...
public class RogueGoblinDeadState : IState
{
    public override void OnEnter(FSM enemy)
    {
        enemy.anim.SetBool("run", false);
